...
void movebydistance(int value) {
  ...
  if (sd == 1) {
    ...
    if (abs(distancego) < 20) {
      speedcontrol(1);             //when close to the target reduce speed to minimise error
    }
    if (abs(distancego) > 20) {
      speedcontrol(4);             //move in higher speed to improve effeciency
    }
    if (abs(distancego)<2){
      sd = 0;
      seq = seq + 1;
