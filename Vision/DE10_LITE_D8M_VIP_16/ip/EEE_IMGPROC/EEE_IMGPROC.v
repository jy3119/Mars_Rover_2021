...
//assign red_detect = ((9'd10 >= hue >= 9'd0) && (8'd255 >= saturation >= 90) && (8'd127 >= value >= 8'd20)) ? 1 : 0;

// Detect green areas
wire green_detect;
assign green_detect = ~red[7] & green[7] & ~blue[7];
//assign green_detect = ((9'd140 >= hue > 9'd81) && (8'd255 >= saturation >= 90) && (8'd127 >= value >= 8'd20)) ? 1 : 0; //hue 165, 130

// Detect blue areas
wire blue_detect;
//assign blue_detect = ~red[7] & ~green[7] & blue[7];
assign blue_detect = ((9'd130 >= hue >= 9'd110) && (8'd255 >= saturation >= 8'd50) && (8'd255 >= value >= 8'd50)) ? 1 : 0; //hue 110, 130 worked well.

//Detect magenta areas
wire magenta_detect;
//assign magenta_detect = ((9'd330 >= hue >= 9'd281) && (8'd100 >= saturation >= 8'd50) && (8'd255 >= value > 8'd45)) ? 1 : 0; //value >=45
assign magenta_detect =((9'd315 >= hue >= 9'd285) && (8'd255 >= saturation >= 8'd100) && (8'd255 >= value > 8'd100)) ? 1 : 0;
...
assign yellow_detect = ((9'd30 >= hue >= 9'd20) && (8'd255 >= saturation >= 8'd100) && (8'd255 >= value > 8'd100)) ? 1 : 0; //value >= 50
...
reg [10:0] x_length, x_center;// y_length;
...
always@(posedge clk) begin
	if (red_detect & in_valid) begin 
		col_state <= 8'b00000000; //0
//		if (x_length > 11'b00000110110) begin 
		detected <= 16'b1;
//		end else begin
//			detected <= 8'b0;
//		end
	end else if (green_detect & in_valid) begin 
		col_state <= 8'b00000001; //1
		detected <= 16'b1;
	end else if (blue_detect & in_valid) begin 
		col_state <= 8'b00000010; //2
		detected <= 16'b1;
	end else if (magenta_detect & in_valid) begin 
		col_state <= 8'b00000011; //3
		detected <= 16'b1;
	end else if (yellow_detect & in_valid) begin 
		col_state <= 8'b00000100; //4
		detected <= 16'b1;
	end else begin
		detected <= 16'b0;
	end
...
//reg signed[15:0] hor_dist; 	//print
//reg[15:0] diag_dist;		//print
//reg [7:0] perp_dist;
//reg [7:0] detected;		//print

//always@(posedge clk) begin
//	if (ball_detect & (x_length > 11'b00000110110)) begin	//value when it is 50cm away 
//		detected <= 8'b00000001;
...
////			end 
//	end else begin
//		detected <= 8'b00000000;
//	end
//end

//Process bounding box at the end of the frame.
reg [1:0] msg_state;
reg [10:0] left, right, top, bottom, x_len_, x_cen_;
reg[7:0] col_, det_;
//reg [15:0] diag_;
//reg signed [15:0] hori_;
reg [7:0] frame_count;
always@(posedge clk) begin
...
//		det_ <= detected;	
	...
	//if (msg_state == 3'b1) msg_state <= 3'b000;
...
always@(*) begin	//Write words to FIFO as state machine advances
	case(msg_state)
		...
		2'b10: begin
			msg_buf_in = {8'b0, col_state, detected};
			msg_buf_wr = 1'b1;
		end
		2'b11: begin
			msg_buf_in = {5'b0, x_length, 5'b0, x_center};
			msg_buf_wr = 1'b1;
		end
//		3'b011: begin
//			msg_buf_in = {16'b0, detected};
//			msg_buf_wr = 1'b1;
//		end
//		3'b100: begin
//			msg_buf_in = {16'b0, hor_dist};
//			msg_buf_wr = 1'b1;
//		end
//		3'b101: begin
//			msg_buf_in = {16'b0, diag_dist};
//			msg_buf_wr = 1'b1;
//		end
	endcase
end
