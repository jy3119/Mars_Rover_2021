...
alt_32 pass_to_esp32;
alt_32 colour_num;

void updateTmp(alt_32 base_addr, alt_32 pass_to_esp32){
	alt_32 tmp = pass_to_esp32;
  ...
  while(1){
       ...
       if((IORD(KEY_BASE,0)&0x0F) == 0x07){
    	   ...
    	   MIPI_BIN_LEVEL(bin_level);
       	 ...
       	 alt_32 pass_to_esp32 = IORD(0x42000, 8); //BUT NEED SELECT JUST THE SECOND WORD
         alt_32 colour_num = IORD(0x42000, 16); //SELECT THE THIRD WORD

         //Write to i2c mem

         alt_putstr("Writing into I2C memory...\n");

         // writes into the 1st word of I2C MEM
         if (colour_num== 0x00000000) //{var obs_colour = 'red';}
         	   {updateTmp(0x40000, pass_to_esp32);}
         if (colour_num== 0x00000001) //{var obs_colour = 'blue';}
         	   {updateTmp(0x40000+4, pass_to_esp32);}
         if (colour_num== 0x00000002) //{var obs_colour = 'green';}
         	   {updateTmp(0x40000+8, pass_to_esp32);}
         if (colour_num== 0x00000003) //{var obs_colour = 'magenta';}
         	   {updateTmp(0x40000+12, pass_to_esp32);}
         if (colour_num== 0x00000004) //{var obs_colour = 'yellow';}
         	   {updateTmp(0x40000+16, pass_to_esp32);}
