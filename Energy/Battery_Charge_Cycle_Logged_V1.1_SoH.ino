...
void loop() {
  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 1:{ //Discharge state (-250mA and  LEDs)
         current_ref = -250;
         if (V_Bat > 2500) { // While not at minimum volts, stay here
           next_state = 1;
           digitalWrite(8,true);
         } else { // If we reach full discharged, move to error
           next_state = 2;
           digitalWrite(8,false);
         }
    ...
    SOH += (current_measure / capacity)*100;
    
    dataString = String(state_num) + "," + String(V_Bat) + "," + String(current_ref) + "," + String(current_measure) + "," + String(SOH); //build a datastring for the CSV file
